...
function get_pl_data() {
  ...
  $.ajax({
    url: api_address + "/api/cases/getmessages/",
    type: 'post',
    dataType: 'json',
    data: {
      'case_id': case_id,
      ...
      if (data.code === 200) {
        ...
        for (let j = 0; j < d.length; j++) {
          ...
          htmlText += '</div>\
              ...
              j].ding_sum + '</span><img src="images/icons/ico_dianzai.png" data-ding-type="1" onclick="ding2(this,' + d[j].id +
            ')"></span>\
...
function q_tj_pl() {
  ...
  var case_id = $('#case_id').val();
  $.ajax({
    url: api_address + '/api/cases/leavemessages',
    type: 'post',
    dataType: 'json',
    data: {
      'case_id': case_id,
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.code == 'add200') {
        ...
        $(e).attr('src', 'images/icons/ico_dianzai_s.png');
